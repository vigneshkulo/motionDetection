...
INCLUDESv:=$(shell pkg-config --cflags opencv)
headers +=-I${INCLUDESv}
CFLAGSv:=-Wall -ggdb
LDFLAGSv:=$(shell pkg-config --libs opencv)

# combine options
CPPFLAGS = ${flags-api} ${flags-${ARCH}} -DARCH=${ARCH} ${headers}
CFLAGS   = ${flags-debug} ${CFLAGSv}
LDFLAGS  = ${flags-${ARCH}}

# how to link against liblitmus
liblitmus-flags = -L${LIBLITMUS} -llitmus ${LDFLAGSv}
