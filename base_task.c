...
/* Motion Tracking Global Variables */
...
int main(int argc, char** argv)
{
	 ...
	 * be_migrate_to_cpu(CPU); // Partitioned EDF
	   be_migrate_to_cluster(0,1); // Cluster EDF	
	  // be_migrate_to_cluster(0,1); // Cluster EDF	
...
int job(void) 
{
	...
	#ifdef DE 
	namedWindow("Threshold difference", CV_WINDOW_NORMAL);
	cvMoveWindow("Threshold difference", 100, 400);
	imshow("Threshold difference", thrImg);
	#endif
...
//	Mat drawing = Mat::zeros( thrImg.size(), CV_8UC3 ); // Has the boundaries only
